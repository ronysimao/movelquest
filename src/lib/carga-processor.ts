/**
 * carga-processor.ts
 * Lógica principal de processamento de cargas (XLSX/PDF com Gemini AI).
 * Exporta a função `processarCarga(cargaId)` que pode ser chamada
 * diretamente de qualquer route handler sem depender de HTTP.
 *
...
import { suggestColumnMappings, extractProductsFromText, applyMapping } from "@/lib/gemini";
...
/**
 * Processa uma carga (XLSX ou PDF) usando IA Gemini.
...
export async function processarCarga(cargaId: number): Promise<ProcessarCargaResult> {
    ...
    try {
        ...
        console.log(`[Carga ${cargaId}] Finalizada! Auto-aprovados: ${processados} | Para revisão: ${paraRevisao}`);

        // 4. Atualizar status da Carga
        const finalStatus = (processados === 0 && paraRevisao === 0) ? "falha" : "sucesso";
        await supabase
            .from("cargas")
            .update({
                status: finalStatus,
        ...
        return { success: true, processed: processados, review: paraRevisao };
...
async function processXlsx(
    ...
    const mapping = await suggestColumnMappings(cleanHeaders);
...
async function processPdf(
    ...
    if (pdfText.trim().length < 50) {
        throw new Error("PDF não contém texto legível suficiente. Pode ser um PDF escaneado sem OCR.");
    }
